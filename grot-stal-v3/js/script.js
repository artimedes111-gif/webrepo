...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if ('IntersectionObserver' in window) {
    ...
    $$('.fade-up').forEach(function (el) { fIO.observe(el); });
  } else {
    $$('.fade-up').forEach(function (el) { el.classList.add('visible'); });
  }
  ...
  if (grid && typeof galleryData !== 'undefined' && galleryData.length) {
    ...
    galleryData.forEach(function (item, index) {
      ...
      img.src = item.src; img.alt = item.title || 'Realizacja'; img.loading = 'lazy';

      var ov = document.createElement('div');
      ov.className = 'gal-overlay';
      ov.innerHTML = '<div class="gal-title">' + (item.title || '') + '</div>' +
      ...
      el.appendChild(img); el.appendChild(ov); el.appendChild(zm);
  ...
  /* ── LIGHTBOX ─────────────────────────────────────────────── */
  var lb = $('#lightbox'), lbBg = $('#lbBackdrop');
  var lbImg = $('#lbImg'), lbCap = $('#lbCap');
  var lbX = $('#lbClose'), lbP = $('#lbPrev'), lbN = $('#lbNext');
  var cur = 0, vis = [];

  window.openLb = function (index) {
    vis = $$('.gal-item:not(.hidden)').map(function (el) { return parseInt(el.dataset.index); });
    cur = Math.max(0, vis.indexOf(index));
    showLb(cur);
    lb.classList.add('on'); lbBg.classList.add('on');
    document.body.style.overflow = 'hidden';
  };

  function showLb(pos) {
    var item = galleryData[vis[pos]]; if (!item) return;
  ...
  function closeLb() {
    lb.classList.remove('on'); lbBg.classList.remove('on');
  ...
  if (lb) {
    ...
    lb.addEventListener('touchend', function (e) {
      if (tx === null) return;
      if (Math.abs(e.changedTouches[0].clientX - tx) > 50)
        e.changedTouches[0].clientX < tx ? nextLb() : prevLb();
      tx = null;
    });
  }
